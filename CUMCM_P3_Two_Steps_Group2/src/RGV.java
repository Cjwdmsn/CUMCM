...
class RGV {
        ...
        for (int i = 0; i < CNCs.length; i++) {
            ...
            switch (CNCs[i].getNextStep()) {
                ...
                case CNC.EJECT_AND_WASH:
                    ...
                    break;
                case CNC.REPAIRED_FIRST_STEP_CNC:
                    if(i >= Constraint.CNCS_COUNT_ONE_ROW) {
                    ...
                    break;
                case CNC.REPAIRED_SECOND_STEP_CNC:
                    if(i >= Constraint.CNCS_COUNT_ONE_ROW) {
                        CNCs[i].setTimeForDoingNextStep(moveTime + CNCs[i].getProcessRemainingTime() + GIVE_OR_EJECT_SOMETHING_TO_SLOW_CNC);
                    } else {
                        CNCs[i].setTimeForDoingNextStep(moveTime + CNCs[i].getProcessRemainingTime() + GIVE_OR_EJECT_SOMETHING_TO_FAST_CNC);
    ...
    private void timeLapse(CNC[] CNCs, int CNCIndex, int time, int shortestElapsedTime) {
        for (int i = 0; i < CNCs.length; i++) {
            ...
            if(i != CNCIndex) {
                ...
                } else if(CNCs[i].getNextStep() == CNC.REPAIRED_FIRST_STEP_CNC) {
                    if(CNCs[i].getProcessRemainingTime() - shortestElapsedTime <= 0) {
                        CNCs[i].setProcessRemainingTime(0);
                ...
                } else if(CNCs[i].getNextStep() == CNC.REPAIRED_SECOND_STEP_CNC) {
                    if(CNCs[i].getProcessRemainingTime() - shortestElapsedTime <= 0) {
                        CNCs[i].setProcessRemainingTime(0);
                        CNCs[i].setNextStep(CNC.GIVE_SOMETHING_SECOND_TIME);
                    } else {
                        CNCs[i].setProcessRemainingTime(CNCs[i].getProcessRemainingTime() - shortestElapsedTime);
                    }
                }
    ...
    private void doSomething(CNC cnc, int time, int cncIndex) {
        switch (cnc.getNextStep()) {
            case CNC.GIVE_SOMETHING_FIRST_TIME:
                //此时该CNC正在等待产品被放入，这种情况出现在第一次处理产品之前或修复损坏的CNC后
                ...
                if(willBreak()) {
                    ...
                    if(whenToBreak + repairTime < time) {
                        ...
                        cncLogRepaired.setOperation(CNC.REPAIRED_FIRST_STEP_CNC);
            ...
            case CNC.GIVE_SOMETHING_SECOND_TIME:
                ...
                if(willBreak()) {
                    int whenToBreak = whenToBreak(false);
                    int repairTime = repairTime();

                    cnc.setProcessRemainingTime(whenToBreak + repairTime);
                    cnc.setNextStep(CNC.REPAIRED_SECOND_STEP_CNC);

                    if(whenToBreak < time) {
                        CNCLog cncLogBreak = new CNCLog();
                        cncLogBreak.setTime(time - whenToBreak);
                        cncLogBreak.setOperation(CNC.BREAK_SECOND_STEP_CNC);
                        cncLogBreak.setIndex(cncIndex);
                        cncLogs.get(cncIndex).add(cncLogBreak);
                    }
                    if(whenToBreak + repairTime < time) {
                        CNCLog cncLogRepaired = new CNCLog();
                        cncLogRepaired.setTime(time - whenToBreak - repairTime);
                        cncLogRepaired.setOperation(CNC.REPAIRED_SECOND_STEP_CNC);
                        cncLogRepaired.setIndex(cncIndex);
                        cncLogs.get(cncIndex).add(cncLogRepaired);
                    }
                } else {
            ...
            case CNC.REPAIRED_FIRST_STEP_CNC:
                cnc.setProcessRemainingTime(0);
                cnc.setNextStep(CNC.GIVE_SOMETHING_FIRST_TIME);
                break;
            case CNC.REPAIRED_SECOND_STEP_CNC:
                cnc.setProcessRemainingTime(0);
                cnc.setNextStep(CNC.GIVE_SOMETHING_SECOND_TIME);
    ...
    private int whenToBreak(boolean isFirstStepCNC) {
        if(isFirstStepCNC) {
            return (int) (Math.random() * (CNC.PROCESS_FOR_TWO_FIRST_STEP));
        } else {
            return (int) (Math.random() * (CNC.PROCESS_FOR_TWO_SECOND_STEP));
        }
    }
